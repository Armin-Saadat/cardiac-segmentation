...
class VxmDense(LoadableModel):
    ...
    def __init__(self,
        ...
        if int_steps > 0:
            self.pos_flow = layers.VecInt(method='ss', name='%s_flow_int' % name, int_steps=int_steps)(self.pos_flow)
            if bidir:
                neg_flow = layers.VecInt(method='ss', name='%s_neg_flow_int' % name, int_steps=int_steps)(neg_flow)

            # resize to final resolution
            if int_downsize > 1:
                self.pos_flow = layers.RescaleTransform(int_downsize, name='%s_diffflow' % name)(self.pos_flow)
                if bidir:
                    neg_flow = layers.RescaleTransform(int_downsize, name='%s_neg_diffflow' % name)(neg_flow)

        # warp image with flow field
        y_source = layers.SpatialTransformer(interp_method='linear', indexing='ij', name='%s_transformer' % name)([source, self.pos_flow])
        if bidir:
        ...
        self.references.pos_flow = self.pos_flow
